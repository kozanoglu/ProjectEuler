...
public class Problem011 {
	...
	public static void main(String[] args) {
		...
		int grid[][] = {
				{ 8, 02, 22, 97, 38, 15, 00, 40, 0, 75, 04, 05, 07, 78, 52, 12,	50, 77, 91, 8 },
		...
		for (int i = 0; i < grid.length; i++) {
			for (int j = 0; j < grid[0].length; j++) {
				...
				if (j + 3 < grid[0].length) {
					horizontal = grid[i][j] * grid[i][j + 1] * grid[i][j + 2]
							* grid[i][j + 3];
				...
				if (i + 3 < grid.length && j + 3 < grid[0].length) {
					diagonal1 = grid[i][j] * grid[i + 1][j + 1]
							* grid[i + 2][j + 2] * grid[i + 3][j + 3];
					max = Math.max(max, diagonal1);
				}
				if (i - 3 >= 0 && j + 3 < grid[0].length) {
					diagonal2 = grid[i][j] * grid[i - 1][j + 1]
							* grid[i - 2][j + 2] * grid[i - 3][j + 3];
